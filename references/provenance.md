...
Extracted from the events module of a multi-location venue-booking system
(Next.js App Router, React, Firestore, Stripe, several currencies, five
languages). The source ran event types, capacity-limited registration, free,
card-paid and wallet-paid tickets, a Stripe webhook, customer cancellation with
wallet credit, and a cleanup cron for unpaid seats. The templates are the
**hardened** version: a three-pass audit (correctness, code quality, operator
usability) found the defects below, and every one is fixed in the code this
skill ships.
...
for the cron. **Shipped:** `checkout.session.expired` → `CHECKOUT_EXPIRED` for
...
admin of any location. **Shipped:** `parseEventInput` whitelist, capacity ≥
...
a new session did not expire the old one — two payable sessions per seat.
...
| Addition | Where |
|---|---|
| No-show deposits: hybrid card hold (Checkout hold within 6 days, saved card + off-session hold otherwise), release on check-in, capture on no-show or late cancel, partial capture, waivers, renewal for long events, cutoff release | [no-show-deposits.md](no-show-deposits.md) |
| Staff check-in with arrival counts | [ui.md](ui.md), [api-routes.md](api-routes.md) |
| Any ISO-4217 currency, USD default, 0/2/3-decimal handling | [money-and-time.md](money-and-time.md) |
...
| Lane/slot blocking during events stays with the host's booking module | integration point only |

## If you are porting the original

Fix in this order — most money at risk first:

1. Unpriced-currency free tickets and the price-based wallet refund (§1).
2. Seat deletion on `payment_intent.payment_failed` (§2), then session expiry (§3).
3. The cleanup race (§4) and the unverified player id (§10).
4. The raw-body admin update and missing location checks (§8).
5. Cancel-event refunds (§9), unpaid-seat release (§11), pay-again (§12), cron auth (§13).
6. Real refunds (§5), wallet atomicity (§6), webhook idempotency (§7).
7. Time zones (§14) and the smaller items (§15).
