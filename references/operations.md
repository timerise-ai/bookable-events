...
| `EVENTS_MANAGE_TOKEN_SECRET` | guest manage links | At least 32 random bytes (`openssl rand -base64 48`); rotating it breaks outstanding links |
...
| `PLACE_HOLD` | Saved card reaches `holdDueAt`; or a failed hold reaches `holdCutoffAt`, then the seat is released |
| `SETTLE` | `HELD` deposit reaches `settleDueAt`, then capture no-shows / release arrivals; renew if the hold would lapse first |
...
| `CAPTURED` deposits whose intent is not `succeeded` | participants vs PaymentIntents | Tick `RETRY_CAPTURE`; if the intent is `canceled`, the hold lapsed, so record `RELEASED` |
